...
bool MotionSkeleton::ParseBVHReferenceFile(FString BVHFileName)
{
	...
	for (FString &Line : Lines)
	{
		...
		else if (Line.StartsWith(TEXT("CHANNELS"), ESearchCase::IgnoreCase) == true)
		{
			...
			if (Line.ParseIntoArray(Words, TEXT(" "), false) >= 4)
			{
				...
				UE_LOG(LogTemp, Warning, TEXT("BONE %s"), *Bones[Bone].Name);
				Bone++;
			}
		}
	} // end of for
...
void MotionSkeleton::BonesSetRotOrder(ChannelOrderEnum RotOrder)
{
	for (BONE &Bone : Bones)
...
bool MotionSkeleton::BonesSetChannels(uint8 XPos, uint8 YPos, uint8 ZPos, uint8 XRot, uint8 YRot, uint8 ZRot)
{	
	for (BONE &Bone : Bones)
	{
...
void MotionSkeleton::BonesSetOffset(int32 X, int32 Y, int32 Z)
{
	for (BONE &Bone : Bones)
	{
		Bone.Offset[0] = X;
...
bool MotionSkeleton::BoneSetOffset(int32 BoneIndex, float X, float Y, float Z)
{
